...
function nullsLastDescComparator(a, b) {
...
function nullsFirstDescComparator(a, b) {
    if (a === null && b === null) return 0;
    if (a === null) return -1;
    if (b === null) return 1;
    if (a === undefined && b === undefined) return 0;
        ...
        for (const sorter of intSorters) {
            describe(`${sorter.name}`, function () {
                ...
                it('should reject null or undefined input containers with TypeError', function () {
                    assert.throws(() => sorter.fn(null, { ...descOptions, nulls: 'last' }), { name: 'TypeError' });
                    assert.throws(() => sorter.fn(undefined, { ...descOptions, nulls: 'last' }), { name: 'TypeError' });
                });

                it('should place nulls last in collections when nulls policy is set to last', function () {
                    const actual = [3, null, 1, null, 2];
                    const expected = [...actual].sort(nullsLastDescComparator);
                    sorter.fn(actual, { ...descOptions, nulls: 'last' });
                    assert.deepStrictEqual(actual, expected);
                });
                ...
                it('should sort null values within a subslice while leaving outer elements untouched', function () {
                    ...
                    const range = expected.slice(1, 5).sort(nullsLastDescComparator);
                    expected.splice(1, range.length, ...range);
                    sorter.fn(actual, { ...descOptions, start: 1, end: 5, nulls: 'last' });
                    assert.deepStrictEqual(actual, expected);
                });
                ...
                it.skip('should reject nulls immediately before mutation when nulls are forbidden', function () {
                    ...
                    assert.throws(() => sorter.fn(actual, { ...descOptions, nulls: 'ignore' }), { name: 'TypeError' });
                ...
                it('should handle sparse arrays by treating uninitialized slots as missing values', function () {
                    ...
                    actual[2] = 1;
                    actual[4] = 2;
                    const expected = Array.from(actual).sort(nullsLastDescComparator);
                    sorter.fn(actual, { ...descOptions, nulls: 'last' });
                    assert.deepStrictEqual(actual, expected);
                });
                ...
                it('should support nulls-first placement when explicitly requested', function () {
                    ...
                    const expected = [...actual].sort(nullsFirstDescComparator);
                    sorter.fn(actual, { ...descOptions, nulls: 'first' });
                    assert.deepStrictEqual(actual, expected);
                });
