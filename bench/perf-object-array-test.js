...
let algorithms = [{
    'name': 'Native ', 'sortFunction': (array) => {
        array.sort(function (a, b) {
            return a.id - b.id;
        });
        return array;
    }, 'floatingPoint': true, 'negative': true,
}, {
    'name': 'RadixBitV2SortObjectByInt32Key', 'sortFunction': (array) => {
        radixBitV2SortObjectByInt32Key(array, (x) => x.id);
        return array;
    }, 'floatingPoint': false, 'negative': true,
}, {
    'name': 'RadixBitSortObjectByInt32Key', 'sortFunction': (array) => {
        radixBitSortObjectByInt32Key(array, (x) => x.id);
        return array;
    }, 'floatingPoint': false, 'negative': true,
}, {
    'name': 'RadixBitSortObjectByFloat64Key', 'sortFunction': (array) => {
        radixBitSortObjectByFloat64Key(array, (x) => x.id);
        return array;
    }, 'floatingPoint': true, 'negative': true,
}, {
    'name': 'QuickBitSortObjectByInt32Key', 'sortFunction': (array) => {
        quickBitSortObjectByInt32Key(array, (x) => x.id);
        return array;
    }, 'floatingPoint': false, 'negative': true,
}, {
    'name': 'PCountSortObjectByInt32Key', 'sortFunction': (array) => {
        pCountSortObjectByInt32Key(array, (x) => x.id);
        return array;
    }, 'floatingPoint': false, 'negative': true, 'range': 2 ** 21,
}, {
    'name': 'QuickBitLowMemSortObjectByInt32Key', 'sortFunction': (array) => {
...
if (fastSort) {
    algorithms.push({
        'name': 'Fast-sort', 'sortFunction': (array) => {
...
if (timsort) {
    algorithms.push({
        'name': 'Timsort', 'sortFunction': (array) => {
            ...
            for (let a = 0; a < algorithms.length; a++) {
                ...
                if (!((generator.floatingPoint && !algorithm.floatingPoint) || (generator.negative && !algorithm.negative) || (algorithm.range && algorithm.range < range))) {
                    if (algorithm.totalElapsed > 0) {
                        ...
                        console.log(`${algorithm.name.padEnd(34)} avg time: ${(avg).toFixed(6).padStart(12)} ms.`);
                    } else {
